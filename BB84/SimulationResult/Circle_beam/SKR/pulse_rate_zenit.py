...
def main():
    ...
    plt.ylabel("Secret Key rate/$10^{-4}$ (bit/pulse)", fontsize=20)
    plt.title("Secret Key rate/$10^{-4}$ (bit/pulse) vs Zenith Angle", fontsize=20)
    plt.grid(True)
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    plt.legend()

    output_path = os.path.join(os.path.dirname(__file__), f'pulse_rate_vs_zenith_all_conditions_{n_s}.png')
