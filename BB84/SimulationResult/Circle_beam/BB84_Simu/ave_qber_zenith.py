...
def rytov_variance(len_wave, theta_zen_rad, H_g, H_atm, Cn2_profile):
    k = 2 * np.pi / len_wave
    sec_zenith = 1 / np.cos(theta_zen_rad)

    def integrand(h):
        return Cn2_profile(h) * (h - H_g)**(5/6)

    integral, _ = quad(integrand, H_g, H_atm, limit=100, epsabs=1e-9, epsrel=1e-9)
...
def fading_loss(gamma, mu_x, mu_y, sigma_x, sigma_y, theta_zen_rad, H_atm, waist, tau_zen, varphi_mod, H_g):
    eta_t = transmissivity_etat(tau_zen, theta_zen_rad)
    sigma_R_squared = rytov_variance(len_wave, theta_zen_rad, H_g, H_atm, cn2_profile)
...
#==================================================================#
def qner_new_infinite(theta_zen_rad, H_atm, waist, tau_zen, varphi_mod, n_s, H_s, H_g):
    
    params = get_beam_jitter_params(condition='weak', theta_d_rad=theta_d_rad, H_s=H_s)
    mu_x = params["mu_x"]
    mu_y = params["mu_y"]
    ...
    def integrand(gamma_mean):
        return fading_loss(gamma_mean, mu_x, mu_y, sigma_x, sigma_y, theta_zen_rad, H_atm, waist, tau_zen, varphi_mod, H_g) * qber_loss(gamma_mean, n_s)
